...
def find_4closest_grid(la, lg):
    ...
    """
    nearest_la1 = (math.floor((la - 24.25)/la_resolution)) * la_resolution + la_min+0.5*la_resolution
    nearest_lg1 = (math.floor((lg + 123.75)/lg_resolution)) * lg_resolution + lg_min+0.5*lg_resolution

    nearest_la2 = (math.floor((la - 24.25)/la_resolution)) * la_resolution + la_min+0.5*la_resolution
    nearest_lg2 = (math.ceil((lg + 123.75)/lg_resolution)) * lg_resolution + lg_min+0.5*lg_resolution
    
    nearest_la3 = (math.ceil((la - 24.25)/la_resolution)) * la_resolution + la_min+0.5*la_resolution
    nearest_lg3 = (math.ceil((lg + 123.75)/lg_resolution)) * lg_resolution + lg_min+0.5*lg_resolution
    
    nearest_la4 = (math.ceil((la - 24.25)/la_resolution)) * la_resolution + la_min+0.5*la_resolution
    nearest_lg4 = (math.floor((lg + 123.75)/lg_resolution)) * lg_resolution + lg_min+0.5*lg_resolution
...
def output_for_Charlie():
    lines = list()
    c.execute("""SELECT center_la, center_lg, pop_density, is_conus FROM gridmap""")
    for row in c.fetchall():
        lines.append("{0},{1},{2},{3}".format(*row))
    with open("grid_map_output.txt", "w") as f:
        f.write("\n".join(lines))
...
def fill_island():
    ...
    for la, lg in c.execute("SELECT center_la, center_lg from gridmap WHERE is_conus = 1 AND population = 0;").fetchall():
        ...
        try:
            ...
            if nearest_water_distance <= grid_radius: # 说明很近的地方有水
                c.execute("""UPDATE gridmap SET is_conus = 0 WHERE 
                center_la = '{0}' AND center_lg = '{1}';""".format(la, lg))
...
def plot_population_contour():
    ...
    dx, dy = 0.05, 0.05
    x = np.array(la).reshape(104, 236)
    y = np.array(lg).reshape(104, 236)
    z = np.array(pop_density).reshape(104, 236)
    z[np.where(z == 0)] = -17000
